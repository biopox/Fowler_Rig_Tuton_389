...
//creates the old window and triangle from the first 1/2 of class
int triangle_win()
{
...
int cube_win()
{
	...
	// ------------------------------------
	OldShader shade("C:\Graphics\shader.vs", "C:\Graphics\shader.fs");
	...
	unsigned char *data = stbi_load("C:\Graphics\Fire.jpg", &w, &h, &nr, 0);
	if (data)
	{
	...
	//link path to immage here
	data = stbi_load("C:\Graphics\Smashing.jpg", &w, &h, &nr, 0);
	if (data)
	{
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, w, h, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
		glGenerateMipmap(GL_TEXTURE_2D);
